...
<header class="sticky top-0 z-20 bg-background-light/95 dark:bg-background-dark/95 backdrop-blur-md px-4 py-3 flex items-center justify-between border-b border-gray-200 dark:border-gray-800 transition-all">
    ...
    <h1 class="text-base font-bold tracking-tight truncate max-w-[60%] text-slate-900 dark:text-white">{{ $repertoire->name }}</h1>
...
<main class="flex-1 w-full max-w-md mx-auto px-4 pb-24 pt-4" x-data="{ allExpanded: false }">
    ...
    <div class="flex items-center justify-between mb-3 px-1">
        ...
        <button 
            @click="allExpanded = !allExpanded; $dispatch('toggle-all', { state: allExpanded })"
    ...
    <div class="flex flex-col gap-3">
        @forelse($repertoire->blocks as $block)
            <div 
                x-data="{ open: false }" 
                @toggle-all.window="open = $event.detail.state"
            ...
            >
                <div class="flex items-center p-4 cursor-pointer hover:bg-gray-50 dark:hover:bg-gray-800/50" @click="open = !open">
                    ...
                    <div class="flex items-center gap-3">
                        <a href="{{ route('blocks.edit', $block->id) }}" class="p-2 text-gray-400 hover:text-primary transition-colors z-10" title="Editar Bloco" @click.stop>
                            <span class="material-symbols-outlined text-xl">edit</span>
                        </a>

                        <span class="material-symbols-outlined text-gray-300 dark:text-gray-600 transition-transform duration-300" 
                ...
                <div x-show="open" x-collapse style="display: none;" class="bg-gray-50 dark:bg-background-dark/50 border-t border-gray-100 dark:border-gray-800">
                            ...
                            <li class="py-3 flex items-start justify-between gap-3">
                                ...
                                @if($song->key)
                                    <div class="flex-shrink-0">
                                        <span class="text-[10px] font-bold bg-white dark:bg-surface-dark border border-gray-200 dark:border-gray-700 px-2 py-1 rounded text-gray-600 dark:text-gray-400 shadow-sm">
                                            {{ $song->key }}
                                        </span>
                                    </div>
                                @endif
